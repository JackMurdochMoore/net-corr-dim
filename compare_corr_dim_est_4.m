...
% results-est-dim-4
% 
% compare_corr_dim_est_4 uses the model
% C(s) = s^D,
% c(s) = s^D - (s - s)^D
% where D is correlation dimension and C(s) (c(s)) is correlation integral (correlation) at distance s.
%
% Example use:
% D00 = 3; N00 = 10000; k00 = 8; numRep = 100; compare_corr_dim_est_4(D00, N00, k00, numRep); 
% 
% 
% Associated with 
...
% and Changgui Gu. 
%
function compare_corr_dim_est_4(D00, N00, k00, numRep)

saveDataFolder = 'results-est-dim-4';
...
DLims = [-Inf, Inf];
[~, ~, ~, codeCell, descriptionCell, ~, ~] = est_corr_dim_4(1, 2, DLims);
...
saveStr = ['est-dim-2_small-world-manhattan_DLim-', num2str(DLims(1)), '-', num2str(DLims(2)), '_', num2str(numRep), 'rep', '_k-', num2str(k), '_', num2str(numD), 'D-', num2str(min(DD)), '-', num2str(max(DD)), '_p-', num2str(p), '_N0-', num2str(N0)];
if ~exist([pwd, '\', saveDataFolder, '\', saveStr, '.mat'], 'file')
    ...
    for iiRep = 1:numRep
        for iiD = 1:numD
            ...
            [DVec, sMaxVec, D2Mat, codeCell, descriptionCell, ~, ~] = est_corr_dim_4(ss, nn, DLims);
...
saveStr = ['est-dim-2_small-world-manhattan_DLim-', num2str(DLims(1)), '-', num2str(DLims(2)), '_', num2str(numRep), 'rep', '_', num2str(num_k), 'k-', num2str(min(kk)), '-', num2str(max(kk)), '_D-', num2str(D), '_p-', num2str(p), '_N0-', num2str(N0)];
if ~exist([pwd, '\', saveDataFolder, '\', saveStr, '.mat'], 'file')
    try
    numFailures = 0;
...
NN0 = [1, 2, 5]'*(10.^(2:2)); NN0 = NN0(:)'; NN0 = [NN0, 10^3]; NN0 = unique(NN0); numN = numel(NN0);
saveStr = ['est-dim-2_small-world-manhattan_DLim-', num2str(DLims(1)), '-', num2str(DLims(2)), '_', num2str(numRep), 'rep', '_k-', num2str(k), '_D-', num2str(D), '_p-', num2str(p), '_', num2str(numN), 'N0-', num2str(min(NN0)), '-', num2str(max(NN0))];
